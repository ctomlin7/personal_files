...
vector<Vector3d> Atoms::read_xyz(const string &filename) {
    string firstline;
    string line;
    int num_atoms;
...
vector<vector<double>> FiniteDifference::analytical_force(Coordinates coordinates) {
    int num_atoms = coordinates.size();
    vector<vector<double>> force_vec(num_atoms, vector<double>(3, 0.0)); // 3D force vectors
    ...
    for (int i = 0; i < num_atoms; ++i) {
        for (int j = i + 1; j < num_atoms; ++j) {
            ...
            for(int i=0; i<coordinates.size(); i++) {
                for(int j=i+1; j<coordinates.size(); j++) {
                    r_vec[i] = coordinates[i][i] - coordinates[j][i];
...
void FiniteDifference::run_FD(string &filename) {
    ...
    for (int i=0; i<h.size(); i++) {
        vector<vector<double>> F_LJ_forward_difference;
        vector<vector<double>> F_LJ_central_difference;
...
void FiniteDifference::printFormattedData(const string & label, const vector<vector<double>>& data, double stepsize) {
